...
//  Ammended by Tim Borer (BBC R&D) to add additional features
...
public: 
  ...
  const detail::multi_array::extent_gen<NumDims> ranges() const {
      typedef detail::multi_array::extent_gen<NumDims> gen_type;
      gen_type array_ranges;
      index base;
      for (unsigned int i=0; i<NumDims; ++i) {
        base = index_bases()[i];
        array_ranges.ranges_[i] =
          typename gen_type::range(base,base+shape()[i]);
      }
      return array_ranges;
  }
